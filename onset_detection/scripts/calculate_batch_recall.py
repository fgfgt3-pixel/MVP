...
for label in labels:
    ...
    event_path = project_root / "onset_detection/data/events/batch" / event_file
    ...
    if detected_events:
        ...
        print(f"[OK] {file} {surge_name} ({strength}): {latency_s:+.1f}s, {len(detected_events)} alerts")
    else:
        detection_results.append({
            **label,
            "detected": False,
...
if total_recall >= 0.60:
    print(f"[OK] 목표 달성! (Recall {total_recall*100:.1f}% >= 60%)")
else:
    gap = 0.60 - total_recall
    needed = int(np.ceil(gap * len(detection_results)))
    print(f"[MISS] 목표 미달 (Recall {total_recall*100:.1f}% < 60%)")
    print(f"       {needed}개 더 탐지 필요")

# 저장
output_path = project_root / "onset_detection/reports/batch_recall_results.json"
