#!/usr/bin/env python3
"""
Dual-Pathway Recall 및 Noise 분석
"""
...
# Dual pathway 결과 로드
results_path = project_root / "onset_detection/data/events/dual_pathway_confirmed.jsonl"
with open(results_path, encoding='utf-8') as f:
    all_events = [json.loads(line) for line in f if line.strip()]

print("="*80)
print("Dual-Pathway Detection 검증 결과")
print("="*80)
...
for label in labels:
    ...
    # Pathway 분석
    sharp_count = len([e for e in detected_events if e.get('pathway') == 'sharp'])
    gradual_count = len([e for e in detected_events if e.get('pathway') == 'gradual'])
    ...
    if detected:
        ...
        pathway_info = f"(Sharp:{sharp_count}, Gradual:{gradual_count})"
        status = "[OK]"
    else:
    ...
    print(f"{status} {file} - {surge_name} ({strength}): {len(detected_events)}개 {pathway_info}")
...
# Pathway별 통계
total_sharp = sum([d['sharp_count'] for d in detected_surges])
total_gradual = sum([d['gradual_count'] for d in detected_surges])
print(f"\nPathway 통계:")
print(f"  Sharp: {total_sharp}개")
print(f"  Gradual: {total_gradual}개")
...
for label in labels:
    ...
    if data_path.exists():
        ...
        # Pathway별 FP
        sharp_fp = len([e for e in file_events if e.get('pathway') == 'sharp' and e['ts'] not in [lbl['start_ts'] for lbl in file_labels]])
        gradual_fp = len([e for e in file_events if e.get('pathway') == 'gradual' and e['ts'] not in [lbl['start_ts'] for lbl in file_labels]])
...
# 결과 저장
output_path = project_root / "onset_detection/reports/dual_pathway_validation.json"
